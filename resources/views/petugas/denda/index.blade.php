...
<div class="p-6 max-w-6xl mx-auto">
            ...
            <table class="min-w-full text-sm divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($nunggak as $d)
                        @php
                            $totalBayar = $d->pembayaran->where('tipe','bayar')->sum('nominal_bayar');
                            $sisa = $d->nominal_tagihan - $totalBayar;
                        @endphp
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="p-3 font-semibold">Rp {{ number_format($d->nominal_tagihan) }}</td>
                            <td class="p-3 text-green-600">Rp {{ number_format($totalBayar) }}</td>
                            <td class="p-3 text-red-600">Rp {{ number_format($sisa) }}</td>
                            <td class="p-3">
                                @if($sisa <= 0)
                            ...
                            <td class="p-3 space-y-1">
                                <button onclick="openModal('bayar{{ $d->id }}')" 
                                    class="w-full bg-green-600 text-white px-2 py-1 rounded text-xs hover:bg-green-700 transition">
                                    Bayar
                                </button>

                                @if($d->jenis == 'hilang')
                                    <button onclick="openModal('revisi{{ $d->id }}')" 
                                        class="w-full bg-red-600 text-white px-2 py-1 rounded text-xs hover:bg-red-700 transition">
                                        Revisi
                                    </button>
                                @endif
                            </td>
    ...
    <div class="bg-white rounded-2xl w-full max-w-md p-6">
        ...
        <form method="POST" action="{{ route('petugas.pembayaran.lunas', $d->peminjaman_id) }}">
            @csrf

            <input type="hidden" name="denda_id" value="{{ $d->id }}">

            <p>Total: <b>Rp {{ number_format($d->nominal_tagihan) }}</b></p>
            <p class="text-green-600">Sudah: Rp {{ number_format($totalBayar) }}</p>
            <p class="text-red-600 mb-3">Sisa: Rp {{ number_format($sisa) }}</p>
            ...
            <div class="flex justify-end gap-2">
                ...
                <button type="submit"
                            ...
                            <div class="bg-white rounded-2xl w-full max-w-md p-6">
                                ...
                                <form method="POST" action="{{ route('petugas.denda.revisi', $d->id) }}">
                                    ...
                                    <input type="number" name="nominal_baru"
                                        class="w-full border border-gray-300 rounded-lg p-2 my-2" placeholder="Nominal Baru" required>

                                    <textarea name="keterangan"
                                        class="w-full border border-gray-300 rounded-lg p-2 mb-3" placeholder="Keterangan"></textarea>

                                    <div class="flex justify-end gap-2">
                                        <button type="button" onclick="closeModal('revisi{{ $d->id }}')"
                                            class="px-3 py-1 bg-gray-300 rounded-lg">Batal</button>
                                        <button class="px-3 py-1 bg-red-600 text-white rounded-lg">Simpan</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    @empty
                        <tr>
                            <td colspan="8" class="text-center p-4 text-gray-500">Tidak ada denda</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="overflow-x-auto bg-white rounded-2xl shadow">
            <table class="min-w-full text-sm divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center p-4 text-gray-500">Tidak ada denda lunas</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
