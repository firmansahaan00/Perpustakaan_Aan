...
<div class="max-w-3xl mx-auto mt-8 p-6 bg-white rounded-2xl shadow-lg">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">
        Proses Retur: {{ $peminjaman->buku->judul_buku }}
    </h2>
    ...
    <form action="{{ route('petugas.pengembalian.proses', $peminjaman->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" class="w-full border rounded-lg px-3 py-2 bg-gray-100" 
                   value="Rp {{ number_format($telat) }}" disabled>
        </div>
        ...
        <div class="mb-4 hidden" id="nominal_lain_container">
            <label class="block text-gray-700 font-medium mb-1">Nominal Denda Lain (Rp):</label>
            <input type="number" name="nominal_lain" 
                   class="w-full border rounded-lg px-3 py-2"
                   min="0" value="0">
        </div>

        {{-- Tombol --}}
        <div class="flex flex-wrap gap-3" id="button_container">
            @if ($telat > 0)
...
function updateUI() {
    const telat = {{ $telat }};
    const dendaLain = dendaSelect.value;

    // tampilkan input nominal
    if (dendaLain === 'hilang' || dendaLain === 'rusak') {
